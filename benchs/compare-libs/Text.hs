...
main = defaultMain
    [ bgroup "break"
        [ bgroup "#english"
            [ bench "foundation" $ whnf (fst . F.break (== '帝')) (F.fromList textEnglish :: F.String)
            , bench "text" $ whnf (fst . T.break (== '帝')) (T.pack textEnglish)
            ]
        , bgroup "#japanese"
            [ bench "foundation" $ whnf (fst . F.break (== '帝')) (F.fromList textJapanese :: F.String)
